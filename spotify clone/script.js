...
async function getFolders() {
    ...
    for (let index = 0; index < as.length; index++) {
        ...
        if(element.href.includes("/songs/")){
            ...
            let b= await fetch(`http://127.0.0.1:3000/projects/spotify%20clone/songs/${folder}/info.json`)
            let r= await b.json()
            ...
            folderSection.innerHTML= folderSection.innerHTML+`<div class="card">
                                                                    ...
                                                                    <img src="http://127.0.0.1:3000/projects/spotify%20clone/songs/${folder}/cover.jpg" alt="${folder}" style="filter: invert(0);">
                                                                    <h2>${r.title}</h2>
                                                                    <p>${r.description}</p>
                                                                </div>`
        }
...
async function getSongs(folder){
    currFolder=folder
    let a=await fetch(`http://127.0.0.1:3000/projects/spotify%20clone/songs/${currFolder}`)//fetching songs from site only
...
const playMusic=(track,pause=false)=>{//pause=false because whenever we open the website then 1st song in library always in playBar
    currSong.src=`/projects/spotify clone/songs/${currFolder}/`+track+`.mp3`
    if(pause==false){
        currSong.play()
