...
async function getFolders() {
    let a= await fetch(`https://github.com/vikhyatcharak/Spotify-Clone/tree/main/spotify%20clone/songs/`)
    ...
    for (let index = 0; index < as.length; index++) {
        ...
        if(element.href.includes("/songs/")){
            ...
            let b= await fetch(`https://github.com/vikhyatcharak/Spotify-Clone/tree/main/spotify%20clone/songs/${folder}/info.json`)
            let r= await b.json()
            ...
            folderSection.innerHTML= folderSection.innerHTML+`<div class="card">
                                                                    ...
                                                                    <img src="https://github.com/vikhyatcharak/Spotify-Clone/tree/main/spotify%20clone/songs/${folder}/cover.jpg" alt="${folder}" style="filter: invert(0);">
                                                                    <h2>${r.title}</h2>
                                                                    <p>${r.description}</p>
                                                                </div>`
        }
...
async function getSongs(folder){
    currFolder=folder
    let a=await fetch(`https://github.com/vikhyatcharak/Spotify-Clone/tree/main/spotify%20clone/songs/${currFolder}`)//fetching songs from site only
...
const playMusic=(track,pause=false)=>{//pause=false because whenever we open the website then 1st song in library always in playBar
    currSong.src=`https://github.com/vikhyatcharak/Spotify-Clone/tree/main/spotify%20clone/songs/${currFolder}/`+track+`.mp3`
    if(pause==false){
        currSong.play()
